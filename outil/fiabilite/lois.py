...
def _intervalle(sol, loi, t1, t2):
    sol.section(f"Probabilité de défaillance dans Δt = [{fmt(t1)} ; {fmt(t2)}]")
...
def exponentielle(lam=None, mttf=None, t=None, t1=None, t2=None, R_cible=None):
    """Loi exponentielle. Donner lam OU mttf. t peut être une valeur ou une liste."""
    sol = Solution("LOI EXPONENTIELLE")
    if lam is None:
        sol.donnee(f"MTTF = {fmt(mttf)}")
        lam = 1 / mttf
        sol.etape("λ = 1/MTTF", lam, "λ")
    else:
        sol.donnee(f"λ = {fmt(lam)}")
        sol.resultats["λ"] = lam
    loi = Exponentielle(lam)
    sol.etape("MTTF = 1/λ", loi.mttf(), "MTTF")
    for ti in _liste(t):
        ...
        sol.etape("f(t) = λ·exp(-λt)", loi.f(ti), f"f({fmt(ti)})")
        sol.etape("λ(t) = λ (constant)", lam)
    ...
    for Rc in _liste(R_cible):
        ...
        sol.etape(f"t = -ln(R)/λ = -ln({fmt(Rc)})/{fmt(lam)}", -math.log(Rc) / lam, f"t(R={fmt(Rc)})")
    return sol


def normale(m, sigma, t=None, t1=None, t2=None, R_cible=None):
    """Loi normale N(m, sigma). Utilise u = (t-m)/sigma et le tableau N1."""
    sol = Solution(f"LOI NORMALE N({fmt(m)}; {fmt(sigma)})")
    loi = Normale(m, sigma)
    sol.etape("MTTF = m", m, "MTTF")
    for ti in _liste(t):
        u = loi.u(ti)
        sol.section(f"t = {fmt(ti)}")
        ...
        sol.etape("f(t) = exp(-u²/2)/(σ√(2π))", loi.f(ti))
        sol.etape("λ(t) = f(t)/R(t)", loi.h(ti), f"λ({fmt(ti)})")
    if t1 is not None and t2 is not None:
    ...
    for Rc in _liste(R_cible):
        ...
        sol.etape("t = m + u·σ", m + u * sigma, f"t(R={fmt(Rc)})")
    return sol
...
def weibull(b=None, theta=None, t0=0.0, t=None, t1=None, t2=None, R_cible=None,
    ...
    sol = Solution(f"LOI DE WEIBULL b={fmt(b)}, θ={fmt(theta)}" + (f", t0={fmt(t0)}" if t0 else ""))
    loi = Weibull(b, theta, t0)
    g = math.gamma(1 + 1 / b)
    sol.etape(f"Γ(1 + 1/b) = Γ({fmt(1 + 1 / b)})", g, "Γ(1+1/b)")
    if t0:
        sol.etape("MTTF = t0 + (θ - t0)·Γ(1 + 1/b)", loi.mttf(), "MTTF")
    else:
        sol.etape("MTTF = θ·Γ(1 + 1/b)", loi.mttf(), "MTTF")
    if b < 1:
    ...
    for ti in _liste(t):
        ...
        sol.etape("f(t) = (b/θ)(t/θ)^(b-1)·exp[-(t/θ)^b]", loi.f(ti))
        sol.etape("λ(t) = (b/θ)(t/θ)^(b-1)", loi.h(ti), f"λ({fmt(ti)})")
    if t1 is not None and t2 is not None:
        _intervalle(sol, loi, t1, t2)
    for Rc in _liste(R_cible):
        sol.section(f"Temps pour lequel R(t) = {fmt(Rc)}")
        tR = t0 + (theta - t0) * (-math.log(Rc)) ** (1 / b)
        sol.etape(f"t = t0 + θ·[-ln R]^(1/b) = t0 + θ·[-ln {fmt(Rc)}]^(1/{fmt(b)})", tR, f"t(R={fmt(Rc)})")
    return sol


def lognormale(m=None, sigma=None, t=None, t1=None, t2=None, R_cible=None,
               mu_y=None, sigma_y=None):
    """Loi log-normale. Donner (m, sigma) de T comme au cours, ou (mu_y, sigma_y)."""
    sol = Solution("LOI LOG-NORMALE")
    if mu_y is None:
        sol.donnee(f"m = {fmt(m)}, σ = {fmt(sigma)} (moyenne et écart-type de T)")
    ...
    sol.etape("MTTF = exp(μy + σy²/2)", loi.mttf(), "MTTF")
    for ti in _liste(t):
        u = loi.u(ti)
        sol.section(f"t = {fmt(ti)}")
    ...
    for Rc in _liste(R_cible):
        ...
        sol.etape("t = exp(μy + u·σy)", math.exp(loi.mu_y + u * loi.sigma_y), f"t(R={fmt(Rc)})")
    return sol
