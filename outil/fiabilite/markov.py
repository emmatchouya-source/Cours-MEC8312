...
def _taux(sol, lam, mttf, mu, mttr):
    if lam is None:
        lam = 1 / mttf
        sol.etape(f"λ = 1/MTTF = 1/{fmt(mttf)}", lam, "λ", "1/T")
    if mu is None:
        mu = 1 / mttr
        sol.etape(f"μ = 1/MTTR = 1/{fmt(mttr)}", mu, "μ", "1/T")
    return lam, mu
...
def element_reparable(lam=None, mu=None, t=None, mttf=None, mttr=None, unite_temps="h"):
    """Élément réparable (2 états, λ et μ constants) ; λ, μ en unite_temps⁻¹."""
    sol = Solution("ÉLÉMENT RÉPARABLE — Markov 2 états (1 marche, 0 panne)", unite_temps)
...
def temps_moyens(MUT=None, MDT=None, MTBF=None, unite_temps="h"):
    """A∞ = MUT/(MDT + MUT) = MUT/MTBF ; ID∞ = MDT/MTBF."""
    sol = Solution("TEMPS MOYENS — disponibilité asymptotique", unite_temps)
    if MTBF is None:
        MTBF = MUT + MDT
        sol.etape("MTBF = MUT + MDT", MTBF, "MTBF", "T")
    elif MUT is None:
        MUT = MTBF - MDT
        sol.etape("MUT = MTBF - MDT", MUT, "MUT", "T")
    elif MDT is None:
        MDT = MTBF - MUT
        sol.etape("MDT = MTBF - MUT", MDT, "MDT", "T")
    sol.etape("A∞ = MUT/MTBF", MUT / MTBF, "A∞")
    sol.etape("ID∞ = MDT/MTBF", MDT / MTBF, "ID∞")
    return sol


def serie_reparable(lams, mus, t=None, unite_temps="h"):
    """Série d'éléments réparables (un réparateur par élément)."""
    sol = Solution("SYSTÈME RÉPARABLE SÉRIE", unite_temps)
    A_inf = 1.0
...
def parallele_reparable(lams, mus, t=None, unite_temps="h"):
    """Parallèle d'éléments réparables (un réparateur par élément, redondance active) :
    disponibilité uniquement."""
    sol = Solution("SYSTÈME RÉPARABLE PARALLÈLE — disponibilité", unite_temps)
    ind = 1.0
    for i, (l, m) in enumerate(zip(lams, mus), 1):
    ...
    for ti in _ts(t):
        ...
        sol.etape(f"Aps(t = {sol.q(ti, 'T')}) = 1 - Π[1 - Ai(t)]", 1 - prod, f"Aps({fmt(ti)})")
    return sol


def parallele2_reparable(lam=None, mu=None, t=None, mttf=None, mttr=None, unite_temps="h"):
    ...
    sol = Solution("DEUX ÉLÉMENTS IDENTIQUES RÉPARABLES EN PARALLÈLE", unite_temps)
    lam, mu = _taux(sol, lam, mttf, mu, mttr)
    ...
    sol.etape(f"MTTFps = (μ + 3λ)/(2λ²)", mttf_s, "MTTFps", "T")
    ...
    d = math.sqrt(B * B - 4 * C)
    s1, s2 = (-B + d) / 2, (-B - d) / 2
    sol.etape("s1 (racine de s² + (3λ+μ)s + 2λ² = 0)", s1, unite="1/T")
    sol.etape("s2", s2, unite="1/T")
    for ti in _ts(t):
        sol.section(f"t = {sol.q(ti, 'T')}")
        ...
        sol.section(f"Comparaison : même système NON réparable, t = {sol.q(ti, 'T')}")
        sol.etape("Rps(t) = 1 - [1 - exp(-λt)]²", 1 - (1 - math.exp(-lam * ti)) ** 2, f"Rps_non_rep({fmt(ti)})")
    sol.etape("MTTFps non réparable = MTTF·(1 + 1/2)", 1.5 / lam, "MTTFps_non_rep", "T")
    return sol


def chaine_markov(transitions, initial, pannes, t=None, etats=None, unite_temps="h"):
    ...
    unite_temps : unité de t (les taux sont en unite_temps⁻¹)
    ...
    sol = Solution(f"CHAÎNE DE MARKOV — {n} états, départ en {initial}", unite_temps)
    ...
    if initial in marche:
        ...
        sol.etape("MTTF = Σ temps moyens passés dans les états de marche", sum(tau), "MTTF", "T")
